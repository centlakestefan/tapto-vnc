...
namespace {
    // Calculate bytes per CPIXEL according to ZRLE/TRLE spec
    ...
    // Encode a single tile (same as TRLE, but no palette reuse - subencoding 127/129 not allowed)
    ...
    // Decode a single tile (same as TRLE, but no palette reuse - subencoding 127/129 not allowed)
    std::vector<U32> decodeTile(const U8* data, size_t& offset, size_t tile_width, size_t tile_height, 
        ...
        // Subencoding 130-255: Palette RLE (but not 127 or 129 in ZRLE)
        if (subencoding >= 130) {
...
std::vector<U8> encodeZrle(const std::vector<U32>& pixels, U16 width, U16 height, const PixelFormat& format) {
    ...
    // First encode tiles without compression (64x64 tiles for ZRLE)
    std::vector<U8> uncompressed;
    
    for (U16 ty = 0; ty < height; ty += 64) {
        for (U16 tx = 0; tx < width; tx += 64) {
            U16 tile_width = std::min(static_cast<U16>(64), static_cast<U16>(width - tx));
            U16 tile_height = std::min(static_cast<U16>(64), static_cast<U16>(height - ty));
            ...
            uncompressed.insert(uncompressed.end(), tile_data.begin(), tile_data.end());
        }
    }
    ...
    int ret = compress(compressed.data(), &compressed_size, 
    ...
    // Build result with 4-byte length prefix
    std::vector<U8> result;
    result.reserve(4 + compressed_size);
    
    // Write length in big-endian format (network byte order)
    result.push_back(static_cast<U8>((compressed_size >> 24) & 0xFF));
    result.push_back(static_cast<U8>((compressed_size >> 16) & 0xFF));
    result.push_back(static_cast<U8>((compressed_size >> 8) & 0xFF));
    result.push_back(static_cast<U8>(compressed_size & 0xFF));
...
std::vector<U32> decodeZrle(const std::vector<U8>& data, U16 width, U16 height, const PixelFormat& format) {
    // Data now contains UNCOMPRESSED tile data (decompression happened in Client using persistent stream)
    // Just decode the tiles directly

    // Decode tiles (64x64 tiles for ZRLE)
    const U8* uncompressed = data.data();
    ...
    for (U16 ty = 0; ty < height; ty += 64) {
        for (U16 tx = 0; tx < width; tx += 64) {
            ...
            // Decode tile
            std::vector<U32> tile_pixels = decodeTile(uncompressed, offset, tile_width, tile_height,
